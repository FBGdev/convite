...
class InviteFlowTest(unittest.TestCase):
    ...
    def test_csrf_and_validation(self):
        ...
        self.assertEqual(self.submit(companion_names=["Beatriz", "Carlos"]).status_code, 200)
        self.assertEqual(self.module.store.records[0]["companion_names"], ["Beatriz", "Carlos"])
        self.assertEqual(self.module.store.records[0]["companions"], 2)
        self.assertEqual(self.module.store.records[0]["children"], 0)

    def test_invite_allows_up_to_six_named_companions(self):
        ...
        self.assertIn('name="companion_names"', page)
        self.assertIn('id="add-companion"', page)
        ...
        self.assertEqual(page.count('data-companion-row'), 6)
    ...
    def test_named_companions_are_saved_and_counted(self):
        names = ["Beatriz", "Carlos", "Daniela", "Eduardo", "Fernanda", "Gabriel"]
        ...
        self.assertEqual(row["companion_names"], names)
        self.login()
        dashboard = self.client.get("/admin").get_data(as_text=True)
        self.assertIn('data-label="Acompanhantes"', dashboard)
        for person in names:
            self.assertIn(person, dashboard)
        ...
        self.assertIn("Sim;Beatriz, Carlos, Daniela, Eduardo, Fernanda, Gabriel;7;", csv_text)

    def test_companion_count_validation(self):
        self.assertEqual(self.submit(bring_wife="yes", wife_name="Beatriz").status_code, 400)
        self.assertEqual(self.submit(companion_names=["Beatriz"] * 7).status_code, 400)
        self.assertEqual(self.submit(companion_names=["A"]).status_code, 400)
        self.assertEqual(self.submit(attending="no", companion_names=["Beatriz"]).status_code, 400)
        invalid = self.submit(phone="123", companion_names=["Beatriz", "Carlos"])
        self.assertEqual(invalid.status_code, 400)
